...
def dataTrainIndex(audioPath, targetWords, PARAMS):
    ...
    for label in listdir(audioPath):
        ...
        for fname in listdir(join(audioPath,label)):
            ...
            # Train or Validation?
            setname  = which_set(fname, PARAMS['validationPercentage'], 10)
...
def inputGenerator(dataset, doAugment, backgrounds, PARAMS,FLAG):
    ...
    while batchStart < epochSize:
        ...
        if FLAG:            
            ...
            labels   = [e['label'].encode('utf-8') for e in batch]
...
def readWavFile(fname):
    w = wave.open(fname, 'r')
    framerate = w.getframerate()  #get sampling rate
    sampwidth = w.getsampwidth()  #get sample size(byte)
    nchannels = w.getnchannels()  #get audio channel(mono:1,stereo:2)
