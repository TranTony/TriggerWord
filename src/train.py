...
if __name__ == '__main__':
    ...
    print('Created {} validation examples (pre-augmentation'.format(len(trainData['validation'])))
    print('Created {} testing examples (pre-augmentation'.format(len(trainData['testing'])))
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(sum(PARAMS['numEpochs'])):            
            ...
            timeStart = timer()
            for batch in util.inputGenerator(trainData['training'],True, backgrounds, PARAMS, 1):                
                _ , batch_loss, batch_accuracy, summary = sess.run([training_op, loss, accuracy, merged], feed_dict={batch_data: batch['features'], batch_labels: batch['labels'], learning_rate: epochLearningRate, isTraining: 1})
                ...
                if batchCount % batchReportInterval == 0:
                    ...
                    writer.add_summary(summary, global_step = batchCount)
                    timeStart = timer()
        ...
        if PARAMS['validationPercentage'] > 0:
            ...
            for batch in util.inputGenerator(trainData['validation'],False, None, PARAMS, 1):                
